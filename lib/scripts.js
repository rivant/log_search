...
module.exports = {
  ...
  getSrcAdapterList: function(params){		
      ...
      if (encrypted) {
				cp.exec("shell/accessConfig.sh "+ params.userID +" "+ params.serverIP +" "+ params.role +" SOURCE "+ params.tmpSrcKey +" "+ encrypted,
					(err, list, stderr) => {
						if (list){	
  ...
  getDstAdapterList: function(params){
      ...
      if (encrypted) {
				cp.exec("shell/accessConfig.sh "+ params.userID +" "+ params.serverIP +" "+ params.role +" DEST "+ params.tmpDstKey +" "+ encrypted,
					(err, list, stderr) => {
						if (list){	
