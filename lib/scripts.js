...
module.exports = {
  ...
  searchLogs: function(params, callBack){
    ...
    this.encrypt(params.userPass, params.tmpSearchKey, (err, encrypted, stderr) => {
      if (encrypted) {        
        let results = cp.exec("shell/access.sh "+ params.userID +" "
          + params.serverIP +" "+ params.source +" "+ params.pattern +" "+ this.range.end +" "
          + this.range.start +" "+ params.tmpSearchKey +" "+ params.dest +" "+ encrypted);
