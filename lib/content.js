module.exports = {
   ...
   toHtml: function(dataStr){
      var html = '<ul>', srcStack = dataStr.slice(2).split('\n'),
          search = new RegExp(this.params.Pattern, 'g');

      if (srcStack.length > 2){
         html += '<li>'+ srcStack[0].trim() +'</li><ul><li>'+ srcStack[1] +'</li><ul>';
         html += '<li>'+ srcStack[2].replace(/(\u000b|\r\u001c)/g, '').trim() +'</li><ul class="destMatch">';
         this.srcTotal++;
      }
   ...
   destToHtml: function(dataStr, html){
      var fileArr = dataStr.match(/.*DEST.log/g), msgArr = dataStr.split('DEST.log');
