module.exports = {
   ...
   srcToHtml: function(val, html){		
      if (val.length > 2){
         ...
         html += '<li>'+ val[0] +'</li>';
         html += '<ul><li><a href="'+ val[1].split('/').pop() +'">'+ val[1] +'</a></li><li>'+ val[5] +'</li><ul>';
   ...
   destToHtml: function(msgStack, html){
      var fileArr = msgStack.match(/\/.*DEST.log[\.]*[APM0-9-]*/g), msgArr = msgStack.split('DEST.log'),
          msg = /(MSH[\s\S]*)\r\u001c/m, timeStamp = /.*(message.*DFDA for.*\.  - [0-9-]* [0-9:,]*).*/;

      if (fileArr){
         for(var idx = 0; idx < fileArr.length; idx++){
            html += '<ul class="destMatch"><li><a href="'+ fileArr[idx].split('/').pop() +'">'+ fileArr[idx] +'</a></li>';
