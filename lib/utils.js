...
module.exports = {
   ...
   nestedKeys(obj, callBack){
      Object.keys(obj).forEach(function(firstKey){
         Object.keys(obj[firstKey]).forEach(function(secondKey){
   ...
   toFile: function(){
      var time = new Date(),
          stream = fs.createWriteStream('./tmp/'+ time.toJSON() +'.html');
      return stream;   
   }
}
