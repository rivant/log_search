module.exports = {
  ...
  uniqueArray: function(concatenatedArrays) {
    for(var i = 0; i < concatenatedArrays.length; ++i) {
      for(var j = i + 1; j < concatenatedArrays.length; ++j) {
        if(concatenatedArrays[i] === concatenatedArrays[j])
          concatenatedArrays.splice(j--, 1);
      }
    }
