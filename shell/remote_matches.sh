...
if [[ -n $DEST ]]; then
   ...
   DEST_REGION_NAME=`sudo -u \#800 ls $ADAPTER_HOME/REGION | grep -E "[A-Z]$DEST_REGION_NUM"`
...
if [ `sudo -u \#800 ls $SRC_PATH 2>/dev/null | wc -l` != 0 ]; then
   SRC_MATCHES=`sudo -u \#800 find $SRC_PATH -type f -mtime +$END_TIME ! -mtime +$START_TIME | grep "${SOURCE}_SOURCE.log" | sort -r | xargs sudo -u \#800 zgrep -n $SEARCH /dev/null| cut -f1-2 -d:`
...
DEST_DATE_MATCHES=`sudo -u \#800 find $DEST_PATH -type f -mtime +$END_TIME ! -mtime +$START_TIME | grep $DEST_LOG | sort -r`
...
do
   ...
   SRC_MSG=`sudo -u \#800 zgrep -e "[:alnum::blank:]*" $SRC_FILE_NAME | sed "${SRC_LINE_NUM},/ACKCODE/!d"`
   CORREL_ID=`echo $SRC_MSG | sed 's/.* COREL ID = \([A-Z0-9]*\) .*/\1/'`
   for DEST_NAME in $DEST_DATE_MATCHES
   do
      DEST_PARTIAL=`sudo -u \#800 zgrep $CORREL_ID $DEST_NAME`
      if [[ -n $DEST_PARTIAL ]]; then
         if [[ -z `echo $DEST_PARTIAL | grep dummy` ]]; then
            DEST_MATCH=`sudo -u \#800 zgrep -e "[:alnum::blank:]*" $DEST_NAME | sed "/$CORREL_ID/,/MSH/!d"`
